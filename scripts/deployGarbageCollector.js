...
async function main() {
  ...
  for (const [name, value] of Object.entries({ ROUTER, WBNB, SCAM_REGISTRY, MINING, VAULT })) {
    if (!value) throw new Error(`Set ${name === "ROUTER" ? "PANCAKE_ROUTER" : name} env var.`);
  }
  // ============================================================
  ...
  const gc = await GarbageCollector.deploy(ADMIN, ROUTER, WBNB, SCAM_REGISTRY, MINING, VAULT);
  ...
  if (network.name !== "hardhat" && network.name !== "localhost") {
    ...
    try {
      await run("verify:verify", {
        address: address,
        constructorArguments: [ADMIN, ROUTER, WBNB, SCAM_REGISTRY, MINING, VAULT],
      });
      console.log("✅ Contract verified on BscScan!");
    } catch (e) {
      console.log("⚠️  Verification failed:", e.message);
      console.log(`   Manual verify: npx hardhat verify --network ${network.name} ${address} ${ADMIN} ${ROUTER} ${WBNB} ${SCAM_REGISTRY} ${MINING} ${VAULT}`);
    }
  }
